...
import com.muthu.jdbctemplate.model.*;
...
public class UserDAO {
	...
	public void save(User user) {
		try {
			LOGGER.info("Entering to save user");
	...
	public void update(User user) {
		LOGGER.info("Entering to update user");
		String sql = "update users set name= ?, email = ? , password = ?  where id = ?";
		Object[] params = new Object[] { user.getName(), user.getEmail(), user.getPassword(), user.getId() };
		int rows = jdbcTemplate.update(sql, params);
		LOGGER.info("No of rows updated - {}", rows);
	}

	public void delete(Integer id) {
		LOGGER.info("Entering to delete user");
